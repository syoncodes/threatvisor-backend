...
# New function to determine if a scan is needed
def is_scan_needed(item, scan_type):
    last_scan_date = item.get('scanned', datetime.min)
    if scan_type == 'passive' and (last_scan_date == datetime.min or last_scan_date < datetime.now() - timedelta(weeks=1)):
        return True
    elif scan_type == 'one-time' and last_scan_date == datetime.min:
        return True
    return False

# Continuously monitor the collection for changes and perform scheduled scans
while True:
    all_ips = []
    for user in collection.find({"endpoints.items.ipAddress": {"$exists": True}}):
        ...
        for endpoint_index, endpoint in enumerate(user["endpoints"]):
            for item_index, item in enumerate(endpoint["items"]):
                if "ipAddress" in item and is_scan_needed(item, item.get('scan', 'passive')):
                    ip = item["ipAddress"]
                    all_ips.append((user_id, endpoint_index, item_index, ip))
                    # Update the last scanned date
                    collection.update_one(
                        {'_id': user_id, f'endpoints.{endpoint_index}.items.{item_index}.ipAddress': ip},
                        {'$set': {f'endpoints.{endpoint_index}.items.{item_index}.scanned': datetime.now()}}
                    )

    if all_ips:
        scan_ports(all_ips)

    # Wait a while before the next iteration
    time.sleep(60)  # Scan hourly for new or scheduled scans
